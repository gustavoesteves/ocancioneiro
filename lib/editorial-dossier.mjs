...
function validateCreators(issues, creators) {
  ...
  creators.forEach((creator, index) => {
    ...
    pushRequiredString(issues, creator.role, `${prefix}.role`);
  });
}
...
function validateEvidenceSources(issues, sources, sourceIds, prefix) {
  if (!Array.isArray(sources) || sources.length === 0) {
    issues.push(`${prefix}.sources deve conter ao menos uma fonte`);
    return;
  }
...
function validateAssets(issues, assets) {
  ...
  assets.forEach((asset, index) => {
    ...
    pushOptionalString(issues, asset.checksumAlgorithm, `${prefix}.checksumAlgorithm`);
    pushIsoDate(issues, asset.generatedAt, `${prefix}.generatedAt`);
  });
}

function validateEditions(issues, editions) {
  if (editions === undefined) return;
...
export function parseEditorialDossier(value) {
  ...
  validateEditions(issues, value.editions);
