...
const finalCurationStatuses = new Set(["aceita", "rejeitada", "inconclusiva"]);
...
export const editorialVocabulary = {
  ...
  editionNotationInstruments: ["piano", "violao"],
  editionNotationKinds: ["lead_sheet", "partitura_instrumental_original"],
  evidenceCriteria: [
    "permanencia",
    "circulacao",
  ...
  researchEventTargetTypes: ["source", "evidence"],
  researchEventTypes: ["correcao", "substituicao"],
  sourceTypes: [
    "manuscrito",
    "edicao_publicada",
    "gravacao",
...
function pushRequiredBoolean(issues, value, field) {
  if (typeof value !== "boolean") {
    issues.push(`${field} deve ser booleano`);
  }
}

function pushPositiveInteger(issues, value, field) {
  if (!Number.isInteger(value) || value < 1) {
    issues.push(`${field} deve ser inteiro positivo`);
  }
}
...
export function decisionRecordHash(decision) {
  const hashableDecision = { ...decision };
  delete hashableDecision.recordHash;
  return createHash("sha256").update(canonicalJson(hashableDecision)).digest("hex");
}
...
function validateCanonicalClaims(issues, claims, evidenceIds) {
  ...
  claims.forEach((claim, index) => {
    ...
    pushRequiredString(issues, claim.justification, `${prefix}.justification`);
    pushOptionalString(issues, claim.decisionId, `${prefix}.decisionId`);
    pushStringArray(issues, claim.evidenceIds, `${prefix}.evidenceIds`);

    if (
      isNonEmptyString(claim.decisionId) &&
      (!Array.isArray(claim.evidenceIds) || claim.evidenceIds.length === 0)
    ) {
      issues.push(`${prefix}.evidenceIds deve conter ao menos uma evidencia relacionada`);
    }

    (claim.evidenceIds ?? []).forEach((evidenceId, evidenceIndex) => {
      if (isNonEmptyString(evidenceId) && !evidenceIds.has(evidenceId)) {
        issues.push(
          `${prefix}.evidenceIds[${evidenceIndex}] referencia evidencia inexistente`,
        );
      }
    });
  });
...
function validateDecisionReviews(issues, reviews, prefix) {
  if (reviews === undefined) return;
  if (!Array.isArray(reviews)) {
    issues.push(`${prefix}.reviews deve ser um array`);
    return;
  }

  reviews.forEach((review, index) => {
    const reviewPrefix = `${prefix}.reviews[${index}]`;
    if (!isRecord(review)) {
      issues.push(`${reviewPrefix} deve ser um objeto`);
      return;
    }

    pushRequiredString(issues, review.reviewedBy, `${reviewPrefix}.reviewedBy`);
    pushIsoDate(issues, review.reviewedAt, `${reviewPrefix}.reviewedAt`);
    pushRequiredString(issues, review.role, `${reviewPrefix}.role`);
    pushRequiredString(issues, review.summary, `${reviewPrefix}.summary`);
    pushRequiredBoolean(
      issues,
      review.conflictOfInterest,
      `${reviewPrefix}.conflictOfInterest`,
    );
    pushOptionalString(
      issues,
      review.conflictDescription,
      `${reviewPrefix}.conflictDescription`,
    );
...
function validateDecisionLocators(issues, locators, prefix) {
  if (locators === undefined) return;
  if (!Array.isArray(locators) || locators.length === 0) {
    issues.push(`${prefix}.locators deve conter localizadores`);
    return;
  }

  locators.forEach((locator, index) => {
    const locatorPrefix = `${prefix}.locators[${index}]`;
    if (!isRecord(locator)) {
      issues.push(`${locatorPrefix} deve ser um objeto`);
      return;
    }

    pushPositiveInteger(issues, locator.measure, `${locatorPrefix}.measure`);
    if (locator.endMeasure !== undefined) {
      pushPositiveInteger(issues, locator.endMeasure, `${locatorPrefix}.endMeasure`);
      if (
        Number.isInteger(locator.measure) &&
        Number.isInteger(locator.endMeasure) &&
        locator.endMeasure < locator.measure
      ) {
        issues.push(`${locatorPrefix}.endMeasure deve ser maior ou igual a measure`);
      }
    }
    pushOptionalString(issues, locator.beat, `${locatorPrefix}.beat`);
    pushOptionalString(issues, locator.note, `${locatorPrefix}.note`);
    pushOptionalString(issues, locator.voice, `${locatorPrefix}.voice`);
  });
}
...
function validateCurationDecisions(issues, decisions) {
  ...
  decisions.forEach((decision, index) => {
    ...
    validateDecisionLocators(issues, decision.locators, prefix);
    validateDecisionReviews(issues, decision.reviews, prefix);
    if (
      decision.status === "aceita" &&
      (!Array.isArray(decision.reviews) || decision.reviews.length === 0)
    ) {
      issues.push(`${prefix}.reviews deve conter ao menos uma revisao`);
    }
    pushOptionalString(issues, decision.recordHash, `${prefix}.recordHash`);
    pushSha256(issues, decision.recordHash, `${prefix}.recordHash`);
    if (
      finalCurationStatuses.has(decision.status) &&
      !isNonEmptyString(decision.recordHash)
    ) {
      issues.push(`${prefix}.recordHash deve selar decisao final publicada`);
    }
    ...
    if (
      ...
      issues.push(`${prefix}.recordHash nao confere com o registro da decisao`);
    }
  });
...
function validateCuration(issues, curation, evidenceIds) {
  ...
  validateCanonicalClaims(issues, curation.canonicalClaims, evidenceIds);
}
...
function validateEvidence(issues, evidence, sourceIds) {
  if (evidence === undefined) return new Set();
...
function validateResearchEvents(issues, events, sourceIds, evidenceIds) {
  if (events === undefined) return;
  if (!Array.isArray(events)) {
    issues.push("researchEvents deve ser um array");
    return;
  }

  const eventIds = new Set();
  events.forEach((event, index) => {
    const prefix = `researchEvents[${index}]`;
    if (!isRecord(event)) {
      issues.push(`${prefix} deve ser um objeto`);
      return;
    }

    pushUniqueId(issues, eventIds, event.id, `${prefix}.id`);
    pushEnum(
      issues,
      event.type,
    ...
    pushRequiredString(issues, event.targetId, `${prefix}.targetId`);
    pushOptionalString(issues, event.replacementId, `${prefix}.replacementId`);
    pushRequiredString(issues, event.reason, `${prefix}.reason`);
    pushRequiredString(issues, event.recordedBy, `${prefix}.recordedBy`);
    pushIsoDate(issues, event.recordedAt, `${prefix}.recordedAt`);

    const targetIds = event.targetType === "source" ? sourceIds : evidenceIds;
    if (isNonEmptyString(event.targetId) && !targetIds.has(event.targetId)) {
      issues.push(`${prefix}.targetId referencia ${event.targetType} inexistente`);
    }

    if (event.type === "substituicao") {
      pushRequiredString(issues, event.replacementId, `${prefix}.replacementId`);
      if (
        isNonEmptyString(event.replacementId) &&
        !targetIds.has(event.replacementId)
      ) {
        issues.push(`${prefix}.replacementId referencia ${event.targetType} inexistente`);
      }
      if (
        isNonEmptyString(event.targetId) &&
        event.targetId === event.replacementId
      ) {
        issues.push(`${prefix}.replacementId deve apontar para outro registro`);
      }
    }
  });
}
...
function validateEditions(issues, editions) {
  ...
  editions.forEach((edition, index) => {
    ...
    if (edition.notationProfile !== undefined) {
      const notationPrefix = `${prefix}.notationProfile`;
      if (!isRecord(edition.notationProfile)) {
        issues.push(`${notationPrefix} deve ser um objeto`);
      } else {
        pushEnum(
          issues,
          edition.notationProfile.kind,
          editorialVocabulary.editionNotationKinds,
          `${notationPrefix}.kind`,
        );
        if (edition.notationProfile.kind === "partitura_instrumental_original") {
          pushEnum(
            issues,
            edition.notationProfile.instrument,
            editorialVocabulary.editionNotationInstruments,
            `${notationPrefix}.instrument`,
          );
          pushRequiredString(
            issues,
            edition.notationProfile.justification,
            `${notationPrefix}.justification`,
          );
        } else {
          if (edition.notationProfile.instrument !== undefined) {
            issues.push(
              `${notationPrefix}.instrument so e permitido para partitura instrumental original`,
            );
          }
          if (edition.notationProfile.justification !== undefined) {
            issues.push(
              `${notationPrefix}.justification so e permitido para partitura instrumental original`,
            );
          }
        }
      }
    ...
    pushIsoDate(issues, edition.validatedAt, `${prefix}.validatedAt`);
  });
...
function validateRights(issues, rights) {
  ...
  pushOptionalString(issues, rights.basis, "rights.basis");
  pushOptionalString(issues, rights.confirmedBy, "rights.confirmedBy");
  pushIsoDate(issues, rights.confirmedAt, "rights.confirmedAt");
...
export function parseEditorialDossier(value) {
  ...
  const evidenceIds = validateEvidence(issues, value.evidence, sourceIds);
  validateResearchEvents(issues, value.researchEvents, sourceIds, evidenceIds);
  validateCuration(issues, value.curation, evidenceIds);
