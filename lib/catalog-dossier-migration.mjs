    ...
    reviews: [
      {
        ...
        reviewedBy: "migracao-catalogo-legado",
        role: "revisao-tecnica",
        summary:
          "Revisao tecnica da migracao; curadoria canonica permanece pendente de bancada humana.",
      },
    ],
    status: "aceita",
  };
...
export function migrateSongToDossier(
  ...
  const parsed = parseCatalog({ songs: [song] }, { allowLegacy: true }).songs[0];
  const asset = {
...
export function migrateCatalogToDossiers(
  ...
  const parsed = parseCatalog(catalog, { allowLegacy: true });
